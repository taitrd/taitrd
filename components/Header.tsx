...
const MotionSpan = dynamic(() => import("@/components/motions/Span"));
const Header = () => {
  return (
        ...
        {headerNavLinks
          ...
          .map((link, key) => (
            <Button
              ...
              variant="ghost"
              asChild
              className="hover:bg-white/40 dark:hover:bg-white/10 hover:backdrop-blur-none"
            >
              <Link
                key={link.title}
                href={link.href}
              ...
              >
                <MotionSpan variants={fadeInUp}>{link.title}</MotionSpan>
              </Link>
            </Button>
          ))}
        {/* <SearchButton /> */}
